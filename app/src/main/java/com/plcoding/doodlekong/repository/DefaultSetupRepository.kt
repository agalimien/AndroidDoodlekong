...
class DefaultSetupRepository @Inject constructor(
    ...
    override suspend fun createRoom(room: Room): Resource<Unit> {
        if (!context.checkForInternetConnection()) {
            return Resource.Error(message = context.getString(R.string.error_internet_turned_off))
        }

        val response = try {
            setupApi.createRoom(room)
        } catch (e: HttpException) {
            return Resource.Error(message = context.getString(R.string.error_http))
        } catch (e: IOException) {
            return Resource.Error(message = context.getString(R.string.check_internet_connection))
        }
        ...
        return if(response.isSuccessful && response.body()?.successful == true) {
            ...
            Resource.Error(message = context.getString(R.string.error_unknown))
        }
    }
